...
namespace ServerControl
{
    ...
    /// </summary>
    public class AnimateInFromBottomProperty : AnimateBaseProperty<AnimateInFromBottomProperty>
    {
        protected override async void DoAnimation(FrameworkElement element, bool value)
        {
            if (value)
                // Animate in
                await element.SlideInFromBottomAsync(FirstLoad ? 0 : 0.3f, keepMargin: false);
            else
                // Animate out
                await element.SlideInToBottomAsync(FirstLoad ? 0 : 0.3f, keepMargin: false);
        }
    }
